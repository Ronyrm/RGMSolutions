...
def get_all_empresas():
    return jsonify(empresa_bolsa_safe.get_all_empresabolsa(tipo=0,limit=-1))
...
def capture_detail_yfinance(name,dt_start,dt_end):
    from App.funcs.funcs import format_date_yyyymmaa
    return jsonify(empresa_bolsa_safe.capture_detail_yfinance(name,format_date_yyyymmaa(dt_start),format_date_yyyymmaa(dt_end)))


@routesempresabolsa.route('/get/cotacao/yahoo/empresas/<name>/<dt_start>')
def capture_val_cotacao_datareader_yahoo(name,dt_start):

    return jsonify(empresa_bolsa_safe.capture_val_cotacao_datareader_yahoo(name,format_date_yyyymmaa(dt_start)))
...
def get_update_divindedos_papel(papel,dt_ini,dt_fim,acao):
    return jsonify(empresa_bolsa_safe.get_update_dividendos_by_papel(papel,
...
def update_all_papel(dt_ini,dt_fim):
    return jsonify(empresa_bolsa_safe.update_data_papel_with_yfinance(format_date_yyyymmaa(dt_ini),
                                                                 format_date_yyyymmaa(dt_fim)))
...
def update_by_papel(idpapel,papel,dt_ini,dt_fim):
    return jsonify(empresa_bolsa_safe.update_data_papel_with_yfinance_by_papel(idpapel,
                                                                 papel,
                                                                 dt_ini,
                                                                 dt_fim))
...
def convert_tickers_csv_statusinvest():
    return jsonify(empresa_bolsa_safe.convert_tickers_csv_statusinvest())


@routesempresabolsa.route('/get/info/ibovespa')
def get_info_Ibovespa():
    return jsonify(empresa_bolsa_safe.get_info_Ibovespa())

@routesempresabolsa.route('/update/prices/ibovespa/<dt_ini>/<dt_fim>')
def update_prices_Ibovespa(dt_ini,dt_fim):
    return jsonify(empresa_bolsa_safe.update_History_Values_Ibovespa(format_date_yyyymmaa(dt_ini),
                                                                format_date_yyyymmaa(dt_fim)))